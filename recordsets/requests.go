...
// NOTE: This struct is copied from gophercloud's implementation so we can
// extend functionality without importing the original package directly in
// exporter code.
...
// ListByZone implements the recordset list request within a specific zone.
...
// ListRecordSets lists recordsets across all zones (Designate API supports this).
func ListRecordSets(client *gophercloud.ServiceClient, zoneID string, opts ListOptsBuilder) pagination.Pager {
	url := recordSetURL(client)
	if opts != nil {
		query, err := opts.ToRecordSetListQuery()
		if err != nil {
			return pagination.Pager{Err: err}
